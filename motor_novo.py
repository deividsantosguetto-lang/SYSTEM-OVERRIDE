"""
motor_novo.py - Motor de produção de cortes virais (SaaS Ready - Nuvem/Docker)
Arquitetura: Gimbal Digital + I/O Volátil (Tempfile) + Tipografia Maximizada (Y: 0.65, Size: 120/130)
"""
...
def _carregar_whisper():
    ...
    if _WHISPER_MODEL is None:
        dispositivo = os.environ.get("WHISPER_DEVICE", "cuda").lower()
        tipo_computacao = os.environ.get("WHISPER_COMPUTE", "int8")
        print(f"    [WHISPER] Carregando faster-whisper small ({dispositivo} | {tipo_computacao})...")
        from faster_whisper import WhisperModel
        try:
            _WHISPER_MODEL = WhisperModel("small", device=dispositivo, compute_type=tipo_computacao)
        except Exception as e:
            print(f"    [AVISO] Falha ao iniciar GPU: {e}. Reduzindo para CPU.")
            _WHISPER_MODEL = WhisperModel("small", device="cpu", compute_type="int8")
        print("    [WHISPER] Modelo alocado.")
...
def encontrar_ffmpeg():
    ...
    for pasta in [BASE_DIR, os.path.join(BASE_DIR, "ffmpeg_extracted"), os.path.join(BASE_DIR, "ffmpeg_extracted", "bin")]:
        ...
        if os.path.exists(ff):
            ...
            return ff, ffprobe
    raise FileNotFoundError("ffmpeg nao encontrado no sistema ou binários locais!")

def encontrar_fonte():
    candidatas = [
        "fonte_viral.ttf", 
        os.path.join(BASE_DIR, "fonte_viral.ttf"),
...
def calcular_eixo_x_dinamico(video_path, total_frames, fps, width, height, target_w=720):
    ...
    clf_frontal = cv2.CascadeClassifier(os.path.join(haar_dir, "haarcascade_frontalface_default.xml"))
    clf_perfil  = cv2.CascadeClassifier(os.path.join(haar_dir, "haarcascade_profileface.xml"))
    clf_corpo   = cv2.CascadeClassifier(os.path.join(haar_dir, "haarcascade_upperbody.xml"))
...
def desenhar_texto_com_contorno(draw, pos, texto, font, cor_texto, espessura=8):
    x, y = pos
...
def desenhar_grupo_karaoke(draw, palavras_grupo, font, width, ty, cor_principal, cor_destaque, tempo_atual=None, tempos_palavras=None):
    ...
    for i, palavra in enumerate(palavras_grupo):
        ...
        desenhar_texto_com_contorno(draw, (x_atual, ty), palavra, font, cor, espessura=8)

        bbox_p = draw.textbbox((0, 0), palavra, font=font)
        x_atual += bbox_p[2] - bbox_p[0]

def obter_fonte_tamanho(fonte_path, texto, largura_max, tamanho_inicial=120):
...
def transcrever_com_whisper(video_path):
    fd, audio_temp = tempfile.mkstemp(suffix=".wav")
    os.close(fd)
...
def adicionar_legenda_e_reframe(video_in, video_out, texto_na_tela="", gancho="", estilo="KARAOKE", cor_hex="#00FF41"):
    ...
    try:
        ...
        r_val, g_val, b_val = int(cor_hex[1:3], 16), int(cor_hex[3:5], 16), int(cor_hex[5:7], 16)
        ...
        if not fps or fps <= 0 or np.isnan(fps): fps = 30.0
        ...
        for i in range(len(grupos) - 1): grupos[i]["fim"] = grupos[i + 1]["inicio"]

        fontes_grupos = []
        for grupo in grupos:
            texto_grupo = " ".join(grupo["palavras"])
            font, _ = obter_fonte_tamanho(fonte_path, texto_grupo, TARGET_W - 80, tamanho_inicial=120)
        ...
        if not out.isOpened():
            ...
            out = cv2.VideoWriter(temp_video_avi, fourcc, fps, (TARGET_W, TARGET_H))
            temp_video = temp_video_avi
        ...
        while True:
            ...
            if texto_na_tela and tempo_atual <= TEMPO_TEXTO_GIGANTE:
                ...
                font_grande, tam_fonte = obter_fonte_tamanho(fonte_path, linha_mais_longa, TARGET_W - 60, tamanho_inicial=130)
                ...
                for idx_linha, texto_linha in enumerate(linhas_texto):
                    ...
                    desenhar_texto_com_contorno(draw, (tx, ty_linha), texto_linha, font_grande, (0, 255, 255), espessura=9)

                frame_out = cv2.cvtColor(np.array(img, dtype=np.uint8), cv2.COLOR_RGB2BGR)
                out.write(frame_out)

            else:
                grupo_atual, font_atual, tempos_atual = None, None, None

                for gi, enumerate_grupo in enumerate(grupos):
                    if enumerate_grupo["inicio"] <= tempo_atual <= enumerate_grupo["fim"]:
                        grupo_atual = enumerate_grupo["palavras"]
                        font_atual = fontes_grupos[gi]
                        tempos_atual = enumerate_grupo.get("tempos")
                        break
...
def fabricar_corte_premium(
    ...
    fd, temp_raw = tempfile.mkstemp(suffix=".mp4")
    os.close(fd)
